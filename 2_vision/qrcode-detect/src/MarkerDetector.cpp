...
MarkerDetector::MarkerDetector(void)
{
    ...
    float markerlenthP =  0.0965;
    float markerlenthN = -0.0965;
...
//	m_markerCorners3d.push_back(cv::Point3f(-0.5f,+0.5f,0));
    ...
    m_markerCorners3d.push_back(cv::Point3f(markerlenthN,markerlenthN,0));
...
// 	imshow("markerCandidate", markerCandidateImg);
	...
	detectMarkers(_frame, _detectedMarkers);
...
bool MarkerDetector::isRotationMatrix(Mat &R)
{
    Mat Rt;
    transpose(R, Rt);
...
Vec3f MarkerDetector::rotationMatrixToEulerAngles(Mat  &R)
{
...
// 		Mat_<float> rotMat(3,3);
        ...
        _detectedMarkers[i].m_rotation = rotationMatrixToEulerAngles(rotMat);
